		...
		<?php
			if(comments_open()){
				...
				if ($smilies_list) {
					...
					switch ($smilies_list[0]) {
						case "bilibili" :
							...
							$bilibili_push_smilies = '<div class="bili-container motion-container"  style="display:flex;">' . push_bili_smilies() . '</div>';
							break;
						case "tieba" :
							$tieba_smilies = '<th onclick="motionSwitch(\'.tieba\')" class="tieba-bar on-hover">Tieba</th>';
							$tieba_push_smilies = '<div class="tieba-container motion-container" style="display:flex;">' . push_tieba_smilies() . '</div>';
							break;
						case "yanwenzi" :
							$menhera_smilies = '<th onclick="motionSwitch(\'.menhera\')" class="menhera-bar on-hover">(=・ω・=)</th>';
							$menhera_push_smilies = '<div class="menhera-container motion-container" style="display:flex;">' . push_emoji_panel() . '</div>';
							break;
						case "custom" :
							$custom_smilies = '<th onclick="motionSwitch(\'.custom\')" class="custom-bar on-hover"> '. iro_opt('smilies_name') .'</th>';
							$custom_push_smilies = '<div class="custom-container motion-container" style="display:flex;">' . push_custom_smilies() . '</div>';
							break;
					}
				...
				$args = array(
					...
					'comment_field' => '<p class="comment-form-tip"><a href="https://segmentfault.com/markdown" target="_blank"><i class="fa-brands fa-markdown" style="color:var(--comment_area_matching);"></i></a> Markdown Supported while <i class="fa-solid fa-code"></i> Forbidden HTML标签 <i class="fa-solid fa-ban"></i></p><div class="comment-textarea"><textarea placeholder="' . __("You are a surprise that I will only meet once in my life", "sakurairo") . ' ..." name="comment" class="commentbody" id="comment" rows="5" tabindex="4"></textarea><label class="input-label">' . __("You are a surprise that I will only meet once in my life", "sakurairo") . ' ...</label></div>
			...
			<div class="commentwrap">
				<?php wp_list_comments(
					array(
						"style" => "ul",
						"type" => "comment",
						"callback" => "kanochan_comment_format",
